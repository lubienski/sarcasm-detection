...
class Processer():
    ...
    def convert_examples_to_features_w_flute(self, examples, label_list, tokenizer, isFlute):
        ...
        #Jane added
        if isFlute:
            all_flute_ids = torch.tensor(flute_ids, dtype=torch.long)
            all_flute_masks = torch.tensor(flute_masks, dtype = torch.long)
            print(f"all flute mask dim: {all_flute_masks.shape}")
            return all_input_ids, all_input_mask, all_know_ids, all_know_mask, all_label_ids,all_flute_ids, all_flute_masks 
        return all_input_ids, all_input_mask, all_know_ids, all_know_mask, all_label_ids
