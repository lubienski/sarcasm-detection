...
import numpy as np
import torch
from models import Bert, KLBert, Bert_concat, ContBert, OurBert
...
def accuracy(out, labels):
    outputs = np.argmax(out, axis=-1).transpose()[0]
...
def main():
    ...
    parser.add_argument('--model_select', default='ContBert',
    ...
    eval_examples = None
    if args.do_train:
        if args.bert_model == "cont_pretrained_bert.ckpt":
            print("loading train data from the rest of the data")
            train_examples = torch.load("90%_train_data_3.pt")
    ...
    if args.do_test:
        ...
        precision, recall, F_score = macro_f1(true_label, pred_outputs)
        # Jane added: save pred_outputs along with true lables, and data
        df = pd.DataFrame()
        df["text"] = [data.text for data in test_examples]
        df["true_labels"] = true_label
        df["pred_labels"] = pred_outputs.argmax(-1).transpose()[0]
        df["logits"] = softmax(pred_outputs, axis = -1).max(-1).transpose()[0]
        df.to_csv(os.path.join(args.output_dir, "test_output.csv"), index = False)
